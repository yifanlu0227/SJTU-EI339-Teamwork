...
class Bee(object):
    ...
    def _list_sub(self,list1,list2):
        l = len(list1)
        new_list = [0]*l
        for i in range(l):
    ...
    def _list_random(self,list1,list2,rand):
        l = len(list1)
        new_list = [0]*l
        for i in range(l):
            new_list[i] = (list1[i]-list2[i])*rand + list1[i]
        return new_list
    ...
    def _random(self, lower, upper):
        ...
        for i in range(len(lower)):
            self.vector.append(self._list_random(upper[i],lower[i],random.random()))
...
class BeeHive(object):
    ...
    of individuals:
        ...
        1. "employees", 引领蜂. 1 for each food source
        2. "onlookers", 跟随蜂
        3. "scouts". 侦查蜂
    ...
    def find_best(self):
        ...
        if (values[index] < self.best): # 越小越好。reward是个负值。
    ...
    def compute_probability(self):
        ...
        if (self.selfun == None):
            self.probas = [0.9 * v / max_values + 0.1 for v in values] # 我们selfun为None，所以用的是这个
        else:
    ...
    def send_employee(self, index):
        ...
        zombee.value = self.evaluate(zombee.vector) # vector对应这actions。 solution vector
        zombee._fitness()
    ...
    def _mutate(self, dim, current_bee, other_bee):
        ...
        return list(np.array(self.population[current_bee].vector[dim])    + \
               (random.random() - 0.5) * 2                 * \
               (np.array(self.population[current_bee].vector[dim]) - np.array(self.population[other_bee].vector[dim])))
