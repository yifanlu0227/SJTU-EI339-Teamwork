...
class MLP(nn.Module):
    ...
    def __init__(self, n_input=10, n_output=8, n_h=2, size_h=128):
        '''
        Specify the neural network architecture

        :param n_input: The dimension of the input
        :param n_output: The dimension of the output。每一维都是差值。还是6维。
...
class DynamicModel(object):
    ...
    def __init__(self,config):
        ...
        if model_config["load_model"]:
            self.model = torch.load(model_config["model_path"],map_location=torch.device('cpu'))
        else:
    ...
    def collect_random_dataset(self):
        ...
        for i in range(self.n_random_episodes):
            ...
            for j in range(self.n_max_steps):
                action = self.env.action_space.sample()  # 随机选择。建立rand数据集
                data_tmp.append(np.concatenate((state_old, action)))  # data_tmp 记录了 state 和 action。 一例state=[0.999857,-0.016918,0.999871,-0.016061,0.919895,-0.890787]。 action=5.425989e-07
                # 这里data_tmp concat 8+2，变成10个元素的np数组
                state_new, reward, done, info = self.env.step(action)  # 有可能报out of bounds，这是在done=True的时候。之前Qube没有报错因为没有小球会掉下去的out of bound情况
                label_tmp.append(state_new - state_old)    # label_tmp 记录了state（observation）的变化值
                if done:
                    break
                state_old = state_new
